...
legal_changes = {
    0: {
        ...
        1,
        2,
    },  # from OPERATE_IN_REV_SERVICE to SELF, PREPARE_FOR_FLIGHT, PARK_AT_BASE
    1: {
        1,
        0,
        2,
    },  # from PREPARE_FOR_FLIGHT to SELF, OPERATE_IN_REV_SERVICE, PARK_AT_BASE
...
def get_action_delay(action, states_status, random=True):
    prop = (
        np.random.randint(1, 100) if random else 1
    )  # 1 always returns tuple of intended action
    some_dict = {
        # 2% of the time the plane requires unplanned service
        0: (0, 0.98) if prop <= 98 else (1, 0.02),
        # 5% of transitions from 2 to 1 fail and remain at 2
...
delay_changes = {
    (0, 0): {
        ...
        1: 0.02,
        2: 0,
    },  # Staying in 0 works in 98% of cases, 2% of the time we are pushed to 1
    (0, 1): {0: 0, 1: 1, 2: 0},
    (0, 2): {0: 0, 1: 0, 2: 1},
    (1, 0): {0: 1, 1: 0, 2: 0},
    (1, 1): {0: 0, 1: 1, 2: 0},
