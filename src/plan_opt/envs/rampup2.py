...
from plan_opt.envs.rampup3 import DEFAULT_CONFIG
import random
import textwrap
import gym
...
class RampupEnv2(gym.Env):
    ...
    @classmethod
    def create(cls, config=DEFAULT_CONFIG, demand=None, timeframe=0, verbose=False):
        ...
        """
        return RampupEnv2(config, demand, timeframe, verbose)

    def __init__(self, config=DEFAULT_CONFIG, demand=None, timeframe=0, verbose=False):
        ...
        self.observation_space = spaces.Box(
            ...
            shape=[self.fleet_size, self.features, self.visibility],
            dtype=np.uint8,
        )
        # Start with past, then add demand
        self.obs_demand = np.append(
            np.array([0] * self.past, dtype=np.uint8),
            (self.demand.data / np.linalg.norm(self.demand.data) * 255).astype("uint8"),
        )
        # Lastly add horizon
        self.obs_demand = np.append(
    ...
    def _retrieve_obs(self):
        ...
        A rolling window of the length of :attr:`visibility`
        """
        obs = []
        start = self.state_time - 1
        end = self.state_time - 1 + self.visibility
        for i in range(self.fleet_size):
    ...
    def reset(self):
        ...
        # self.done = bool(self.state_time == (self.timeframe - self.past - 1))
    ...
    def _set_initial_state(self, initial_state_status=None):
        ...
        if initial_state_status is not None:
            # Dummy var to 1 for chosen initial status and the previous entry :)
            # TODO: for loop to change past if it works
            self.obs_dummy_status[initial_state_status - 1][0] = 1
